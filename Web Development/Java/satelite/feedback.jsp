...
<div class="container">
    ...
    <form action="feedback.jsp" method="post">
        ...
        <div class="input_box">
            ...
            <textarea name="r3" placeholder="Write your review here..." required></textarea>
        </div>

        <button type="submit" name="s1" class="btn-submit"><i class="fas fa-paper-plane"></i> Submit</button>
    </form>
</div>
...
<%
    if(request.getParameter("s1") != null) {
        ...
        String productReview = request.getParameter("r3");
        ...
        try {
            ...
            String sql = "INSERT INTO feedback_details (satellite_name, satellite_model, product_review) VALUES (?, ?, ?)";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, satelliteName);
            pstmt.setString(2, satelliteModel);
            pstmt.setString(3, productReview);

            int rowsInserted = pstmt.executeUpdate();
            if(rowsInserted > 0) {
                out.println("<script>alert('Feedback submitted successfully!');</script>");
            } else {
                out.println("<script>alert('Feedback submission failed.');</script>");
            }
        } catch (Exception e) {
